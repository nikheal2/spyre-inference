...
def test_cpu_block_mask_matches_upstream(patch_embeds_list):
    """The CPU stand-in must reproduce upstream's mask exactly.

    Upstream reads only `dtype` and the two leading dims off `patch_embeds`, so a
    same-shaped CPU tensor yields an identical mask. A device tensor cannot be built
    here, so this pins the value equivalence; the device path is covered by the
    two-image e2e test.
    """
    from transformers.models.pixtral import modeling_pixtral
